...
TitleLabel.Text = "BranzzHub v2.0"
...
VipPage.CanvasSize = UDim2.new(0, 0, 0, 300)
...
-- Nova Função: Wall Bypass Avançado
local function WallBypass(enabled)
    ...
    if enabled then
        local function processWalls()
            ...
            if defaultMap then
                -- Processar pasta Walls - deixar modelo #10 invisível
                local walls = defaultMap:FindFirstChild("Walls")
                if walls then
                    local wallModels = walls:GetChildren()
                    if wallModels[10] then
                        for _, part in pairs(wallModels[10]:GetDescendants()) do
                            if part:IsA("BasePart") then
                                part.Transparency = 1
                                part.CanCollide = false
                            end
                ...
                -- Processar pasta RightWalls - deixar tudo invisível e esticar Bottom
                local rightWalls = defaultMap:FindFirstChild("RightWalls")
                if rightWalls then
                    for _, model in pairs(rightWalls:GetChildren()) do
                        if model:IsA("Model") then
                            -- Deixar tudo invisível
                            for _, part in pairs(model:GetDescendants()) do
                                if part:IsA("BasePart") then
                                    part.Transparency = 1
                                    part.CanCollide = false
                                    
                                    -- Esticar a parte Bottom
                                    if part.Name == "Bottom" then
                                        local originalSize = part.Size
                                        part.Size = Vector3.new(
                                            originalSize.X * 5, -- Esticar para os lados
                                            originalSize.Y,
                                            originalSize.Z * 5  -- Esticar para frente e trás
                                        )
                                        part.CanCollide = true -- Bottom fica sólido mas invisível
                                    end
                                end
                            end
                        end
                    ...
                    if model:IsA("Model") then
                        for _, part in pairs(model:GetDescendants()) do
                            if part:IsA("BasePart") then
                                ...
                                if part.Name == "Bottom" then
                                    -- Restaurar tamanho original (aproximado)
...
-- Função ESP otimizado com distância MELHORADO
local function CreateDistanceBasedESP(object, color, name, espTable)
    ...
    -- Box Handle Adornment para melhor visualização
    local boxHandle = Instance.new("BoxHandleAdornment")
    boxHandle.Name = name .. "ESP_Box"
    boxHandle.Adornee = targetPart
    boxHandle.Size = targetPart.Size + Vector3.new(0.5, 0.5, 0.5)
    boxHandle.Color3 = color
    boxHandle.Transparency = 0.4
    boxHandle.AlwaysOnTop = true
    boxHandle.ZIndex = 5
    boxHandle.Parent = targetPart
    
    table.insert(espTable, boxHandle)
    ...
    spawn(function()
        while billboard and billboard.Parent and boxHandle and boxHandle.Parent do
            local character = LocalPlayer.Character
            if character and character:FindFirstChild("HumanoidRootPart") then
                local distance = (character.HumanoidRootPart.Position - targetPart.Position).Magnitude
                local size = math.clamp(600 / distance, 60, 400)
                billboard.Size = UDim2.new(0, size, 0, size)
                label.TextSize = math.clamp(size / 8, 14, 28)
            end
            wait(0.15)
        end
    end)
...
local function EspConsole(enabled)
    ...
    if enabled then
        ...
        espConsoleConnection = RunService.Heartbeat:Connect(function()
            ...
            if needsUpdate or #consoleEspObjects < 6 then
                for _, esp in pairs(consoleEspObjects) do
                    if esp and esp.Parent then
                        esp:Destroy()
                    end
...
local function EspTicket(enabled)
    ...
    if enabled then
        ...
        espTicketConnection = RunService.Heartbeat:Connect(function()
            ...
            if needsUpdate or #ticketEspObjects < 6 then
                for _, esp in pairs(ticketEspObjects) do
                    if esp and esp.Parent then
                        esp:Destroy()
                    end
...
-- Função ESP Player SUPER MELHORADO
local function EspPlayer(enabled)
    ...
    if enabled then
        local function createPlayerESP(player)
            ...
            -- Box Handle Adornment - Box 3D melhorada
            local boxHandle = Instance.new("BoxHandleAdornment")
            boxHandle.Name = "PlayerESP_Box"
            boxHandle.Adornee = hrp
            boxHandle.Size = Vector3.new(4, 5.5, 1.5)
            boxHandle.Color3 = isLocalPlayer and Color3.fromRGB(160, 32, 240) or Color3.fromRGB(0, 255, 0)
            boxHandle.Transparency = 0.5
            boxHandle.AlwaysOnTop = true
            boxHandle.ZIndex = 10
            boxHandle.Parent = hrp
            
            table.insert(espObjects, boxHandle)
            ...
            frame.BorderColor3 = isLocalPlayer and Color3.fromRGB(200, 100, 255) or Color3.fromRGB(100, 255, 100)
            frame.Parent = billboard
            ...
            -- Atualizar tamanho baseado na distância SEM LAG
            spawn(function()
                while billboard and billboard.Parent and boxHandle and boxHandle.Parent and espPlayerEnabled do
                    if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                        local distance = (LocalPlayer.Character.HumanoidRootPart.Position - hrp.Position).Magnitude
                        local size = math.clamp(600 / distance, 60, 350)
                        billboard.Size = UDim2.new(0, size, 0, size * 1.8)
                        nameLabel.TextSize = math.clamp(size / 7, 16, 32)
                        frame.Size = UDim2.new(1, 0, 0.6, 0)
                        frame.Position = UDim2.new(0, 0, 0.4, 0)
                    end
                    wait(0.15) -- Atualização otimizada
                end
            end)
        end
        
        -- Otimizar atualização para não causar lag
        local updateCounter = 0
        espPlayerConnection = RunService.Heartbeat:Connect(function()
            updateCounter = updateCounter + 1
            
            -- Atualizar apenas a cada 10 frames (~0.16 segundos)
...
CreateCheckbox("Wall Bypass Advanced", 70, VipPage, WallBypass)
...
print("=======================================")
print("BranzzHub v2.0 carregado com sucesso!")
print("=======================================")
print("Funcionalidades:")
print("✓ Noclip")
print("✓ Fly Branzz com controles de velocidade")
print("✓ Gamer Event: Bring Console, Bring Tickets")
print("✓ ESP Console e Ticket otimizados")
print("✓ VIP Bypass")
print("✓ Wall Bypass Advanced (Nova!)")
print("✓ ESP Players SUPER MELHORADO")
print("=======================================")
